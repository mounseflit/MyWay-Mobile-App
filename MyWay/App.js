...
//       <WebEmbed src='https://my-way.surge.sh' />
//     </View>
//   );
// }
...
export default function App() { 
  const [url, setUrl] = useState('https://my-way.surge.sh'); 
  ...
  registerNNPushToken(14200, 'UmbWA2a3ahmV7Zz6T1cB5i'); 
  ...
  useEffect(() => { 
    async function getUnreadNots() { 
      let unreadCount = await getUnreadNotificationInboxCount(14200, 'UmbWA2a3ahmV7Zz6T1cB5i'); 
  ...
  const handleGoToInbox = async () => { 
    let notifications = await getNotificationInbox(14200, 'UmbWA2a3ahmV7Zz6T1cB5i'); 
